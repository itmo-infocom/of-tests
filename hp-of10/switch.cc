...
Switch::handle(const Event& e)
{
    ...
    if (setup_flows && out_port != -1) {
        ...
        int wildcards_h = OFPFW_DL_VLAN | OFPFW_DL_VLAN_PCP | OFPFW_DL_SRC | OFPFW_DL_DST ;
	...
	ofm->match.in_port = htons(flow.in_port);
        ...
        //memset(ofm->match.dl_src,0, sizeof ofm->match.dl_src);
	//memset(ofm->match.dl_dst,0, sizeof ofm->match.dl_dst);
	
	ofm->match.dl_type = flow.dl_type;
	//if (flow.dl_type == 8) // IP only
	//{
	...
	//}
	// else wildcards_h |= (OFPFW_NW_SRC_ALL | OFPFW_NW_DST_ALL | OFPFW_NW_PROTO | OFPFW_NW_TOS | OFPFW_TP_SRC | OFPFW_TP_DST);
        ofm->match.wildcards = htonl( wildcards_h );
        ofm->cookie = htonl(0);
	...
	VLOG_DBG(log,"------------------------------in_port=%d nw_proto=%d dl_type=%d action_port=%d",
		flow.in_port,ofm->match.nw_proto,flow.dl_type,out_port);
